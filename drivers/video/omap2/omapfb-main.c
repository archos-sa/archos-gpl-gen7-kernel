...
static void fill_fb(struct fb_info *fbi)
{
	...
	const short w = var->xres;
	const short h = var->yres;
...
int check_fb_var(struct fb_info *fbi, struct fb_var_screeninfo *var)
{
	...
	var->vmode = display->panel->timings.interlaced ? 
...
int omapfb_apply_changes(struct fb_info *fbi, int init)
{
	...
	for (i = 0; i < ofbi->num_overlays; i++) {
		...
		/* at init time, make all overlays opaque */
		if (init)
		...
		struct fb_info *fbi)
{
	...
	if (var->xoffset != fbi->var.xoffset ||
	    var->yoffset != fbi->var.yoffset || var->activate & FB_ACTIVATE_FORCE) {
...
/* turn off lcd */
static void omapfb_early_suspend(struct early_suspend *es)
{
	int i, r;
	struct omapfb2_device *fbdev = container_of(es,
					 struct omapfb2_device, early_suspend);

	/* blank the screens */
	for (i = 0; i < fbdev->num_fbs; i++) {
		r = omapfb_blank(FB_BLANK_POWERDOWN, fbdev->fbs[i]);
		if (r) {
			dev_err(fbdev->dev, "failed to blank\n");
			return;
		}
	}
...
static void omapfb_early_resume(struct early_suspend *es)
{
	int i, r;
	struct omapfb2_device *fbdev = container_of(es,
					 struct omapfb2_device, early_suspend);
	...
	/* unblank the screens */
	for (i = 0; i < fbdev->num_fbs; i++) {
		r = omapfb_blank(FB_BLANK_UNBLANK, fbdev->fbs[i]);
		if (r) {
			dev_err(fbdev->dev, "failed to unblank\n");
			return;
		}
	}
}
...
static int fbinfo_init(struct omapfb2_device *fbdev, struct fb_info *fbi)
{
	...
	if (display) {
		...
		case 24:
			var->bits_per_pixel = 32;
			var->nonstd         = OMAPFB_COLOR_ARGB32;
			break;
		default:
...
static int omapfb_create_framebuffers(struct omapfb2_device *fbdev)
{
	...
	for (i = 0; i < CONFIG_FB_OMAP2_NUM_FBS; i++) {
		...
		/* Make sure that an early context release will not release ctx 0 */
		ofbi->region.vrfb.context = -1;
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	fbdev->early_suspend.suspend = omapfb_early_suspend;
	fbdev->early_suspend.resume = omapfb_early_resume;
